...
public interface JPAWaitingQueueRepository extends JpaRepository<WaitingQueue, Long> {
    ...
    @Modifying
    @Query("UPDATE WaitingQueue wq SET wq.status = 'PROGRESS', wq.processedAt = :processedAt, wq.expiredAt = :expiredAt WHERE wq.id IN :queueList")
    ...
    void updateExpire(@Param("updateStatus") WaitingQueueStatus updateStatus, @Param("currTime") LocalDateTime currTime);
